...
for emissor in emissores:
    # emissor = (91.247,694.027) ** para testar só o primeiro
    print("zerando o detector")
    detectors = np.array([[0 for k in range(1792)]for j in range(2048)])
    print("iniciando o emissor "+str(emissor))
    ...
    for xDetector in np.arange(-143.29,143.43,0.14):
        ...
        for yDetector in np.arange(0.7,250.95,0.14):
            ...
            if(aMax<= aMin):
                # print('para td, n intercepta no i='+ str(iDetector)+" e j="+str(jDetector))
                ignorados += 1
                jDetector += 1
            ...
            detectors[iDetector][jDetector] = d
            jDetector += 1
        print(str(emissor) + " i= " + str(iDetector) + " feito, com "+str(ignorados)+" j ignorados")
        iDetector += 1
            # print(str(d))
    print("emissor " + str(iEmissor) + " feito")
    
    iEmissor+=1
    print("colocando valores entre 0 e 255: ")

    img = detectors/(detectors.max()/255)
    
    print("criando a imagem")

    img = Image.fromarray(img)
    img = img.convert("L")
    img.save("emissor-" + str(iEmissor) + ".jpg")
